...
#				dragging.get_node("Sprite-top").rotation_degrees += 5
			...
			if dragging.padre != null:
				if (typeof(dragging.padre) != TYPE_INT or dragging.padre != 1):
					padre_nombre = dragging.padre.nombre
			
			var hijo_nombre = "null"
			if dragging.hijo != null:
				hijo_nombre = dragging.hijo.nombre
			
			print("Padre: ", padre_nombre, ", Hijo: ", hijo_nombre)
					...
					if dragging.hijo == null or true:
						...
						for s in slots:
							if s != dragging.get_node("Sprite-top/Flechita"):
								...
								if dist < lock_distance:
									if s.get_parent().get_parent().hijo == null or s.get_parent().get_parent().hijo == dragging:
										dragging.global_position = s.global_position
									
									if s.get_parent().get_parent().hijo == null:
										s.get_parent().get_parent().hijo = dragging
										dragging.padre = s.get_parent().get_parent()
								else:
									if s.get_parent().get_parent().hijo == dragging:
										s.get_parent().get_parent().hijo = null
										dragging.padre = null
