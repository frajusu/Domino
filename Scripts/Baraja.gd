...
func draw_cards(n):
	if get_parent().get_parent().draws_actuales > 0:
		...
		if keys.size() != mazo_original.keys().size():
...
func _input(event):
	if get_parent().get_parent().baraja_activa == "baraja" or (get_parent().get_parent().baraja_activa == "baraja_especial" and is_over_drop_zone()):
...
#					var actual = dragging.hijo
#					while actual != null:
#						if !actual.mouse_over_area(get_tree().root.get_node("Game/Viewport/GameArea/Area2D"), 1):
#							actual.desaparecer()
#						actual.get_node("Sprite-top").rotation_degrees += 5
#						actual = actual.hijo
