	...
	if !Global.fijar_dominos:
		...
		if dragging:
			if dragging.get_node("Sprite").visible == false:
				if dragging.slot_lockeado == null or true:
					...
					for s in slots:
						if s != dragging.get_node("Sprite-top/Flechita"):
							...
							if dist < lock_distance:
								if s.get_parent().get_parent().slot_lockeado == null or s.get_parent().get_parent().slot_lockeado == dragging:
									dragging.global_position = s.global_position + Vector2(2, 1)
								
								if s.get_parent().get_parent().slot_lockeado == null:
									s.get_parent().get_parent().slot_lockeado = dragging
									dragging.lockeado = s.get_parent().get_parent()
							else:
								if s.get_parent().get_parent().slot_lockeado == dragging:
									s.get_parent().get_parent().slot_lockeado = null
									dragging.lockeado = null
