...
var domino = preload("res://Scenas/Normals_tienda.tscn")
...
#		s.name = nombre
#		add_child(s)
#		s.global_position = Vector2(913, -326)
#
#
...
func draw_cards(n, sacar_stamps = false):
	...
	for _i in range(n):
		...
		s.name = nombre
		add_child(s)
		s.global_position = Vector2(890, -256)
...
func _physics_process(_delta):
	...
	if get_parent().get_parent().get_parent().baraja_activa == "baraja" or get_parent().get_parent().get_parent().baraja_activa == "baraja_especial":
		...
		if hovered_card != last_hovered_card and !dragging:
			...
			if hovered_card and hovered_card != arrastrado and !get_parent().get_node("Baraja_stamps").dragging and !get_parent().get_node("Baraja_specials").dragging:
