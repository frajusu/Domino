...
func _input(event):
	if get_tree().root.has_node("Game") and get_tree().root.get_node("Game").baraja_activa == "baraja_amuletos":
		if event is InputEventMouseButton:
			if get_parent().get_parent().get_parent().get_parent().get_parent().name != "xd":
				# Click izquierdo
			...
			if get_parent().get_parent().get_parent().get_parent().get_parent().name == "xd":
				if event is InputEventMouseButton and event.pressed:
					if event.button_index == BUTTON_RIGHT or event.button_index == BUTTON_LEFT:
						for s in get_children():
							if s != arrastrado and s.mouse_dentro_area():
								var idx = s.get_index()
								var total = get_child_count()
								
								var idx_objetivo = idx
								if event.button_index == BUTTON_LEFT:
									idx_objetivo = (idx - 1 + total) % total
								else:
									idx_objetivo = (idx + 1) % total
								
								var nodo_a = get_child(idx)
								var nodo_b = get_child(idx_objetivo)
								
								move_child(nodo_a, idx_objetivo)
								move_child(nodo_b, idx)
								
								# nodos vinculados
								var padre = nodo_a.nodo_vinculado.get_parent()
								var a1 = nodo_a.nodo_vinculado
								var b1 = nodo_b.nodo_vinculado
								
								var ia = a1.get_index()
								var ib = b1.get_index()
								
								padre.move_child(a1, ib)
								padre.move_child(b1, ia)
								
								# array
								var arr = get_tree().root.get_node("Game").amuletos_tenidos
								var tmp = arr[idx]
								arr[idx] = arr[idx_objetivo]
								arr[idx_objetivo] = tmp
			
			
			if get_parent().get_parent().get_parent().get_parent().get_parent().name == "xd" and false:
							...
							if s != arrastrado:
								...
								if s.mouse_dentro_area():
									...
									for k in get_children():
										...
										if cont == get_child_count():
											var idx_a = s.get_index()
											var idx_b = cont - 1
											
											var nodo_a = get_child(idx_a)
											var nodo_b = get_child(idx_b)
											
											move_child(nodo_a, idx_b)
											move_child(nodo_b, idx_a)
											
											var otro_padre = nodo_a.nodo_vinculado.get_parent()
											var nodo_a1 = nodo_a.nodo_vinculado
											var nodo_b1 = nodo_b.nodo_vinculado
											
											var idx_a1 = nodo_a1.get_index()
											var idx_b1 = nodo_b1.get_index()
											
											otro_padre.move_child(nodo_a1, idx_b1)
											otro_padre.move_child(nodo_b1, idx_a1)
