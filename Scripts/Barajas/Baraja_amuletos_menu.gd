...
func _input(event):
	if get_tree().root.has_node("Game") and get_tree().root.get_node("Game").baraja_activa == "baraja_amuletos" and get_parent().get_parent().get_parent().get_parent().get_parent().name != "xd":
