...
var layout_width = 850     # ancho máximo del layout
var spacing = 10           # separación mínima
var velocidad_layout = 40  # suavizado


func draw_cards(color: String) -> void:
	mazo_actual = get_parent().get_parent().get_parent().get_node("Baraja").mazo_original.duplicate()
	...
	# Crear cartas poco a poco
	yield(_crear_cartas_gradualmente(cartas_color), "completed")


func _crear_cartas_gradualmente(cartas_color: Array):
	for nombre in cartas_color:
		...
		if get_parent().get_parent().get_parent().get_node("Baraja").mazo_actual.has(nombre):
			s.existente = true
		else:
			s.existente = false
		
		# Esperar un frame antes de seguir con la siguiente carta
