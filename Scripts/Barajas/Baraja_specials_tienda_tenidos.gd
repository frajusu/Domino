...
var domino = preload("res://Scenas/Specials_tienda_tenidos.tscn")
...
var layout_width = 330     # ancho máximo del layout
var spacing = 10           # separación mínima
var velocidad_layout = 10  # suavizado



func draw_cards(n, nombre_especifico):
	print(nombre_especifico)
	...
	# Si se pasa un nombre especifico, ignorar n y crear solo ese
	if nombre_especifico != null:
		if not mazo_original.has(nombre_especifico):
			print("⚠️ No existe el domino llamado: %s" % nombre_especifico)
			return
		n = 1
	else:
		if n > keys.size():
	...
	for _i in range(n):
		...
		if nombre_especifico != null:
			nombre = nombre_especifico
		else:
			# elegir por chance
			var total_chance = 0
			for nom in keys:
				total_chance += mazo_actual[nom]["chance"]
			...
			for nom in keys:
				acumulado += mazo_actual[nom]["chance"]
				if roll <= acumulado:
					nombre_seleccionado = nom
					break
		...
		s.nombre = nombre_corregido
		s.name = nombre
		s.yo = mazo_actual[nombre]
		add_child(s)
		s.global_position = Vector2(1060, 208)
		...
		if nombre_corregido == "Sniper" or mazo_actual[nombre]["BG"].x == 4800:
		...
		# si se pidio un nombre especifico, salir tras crear uno
		if nombre_especifico != null:
			break
...
func _physics_process(_delta):
	if dragging != null and get_parent().get_parent().get_parent().get_parent().get_parent().estacion_actual != "tienda":
		dragging = null
	
	mover_cartas()
	
	if get_parent().get_parent().get_parent().get_parent().get_parent().baraja_activa == "baraja_especial":
