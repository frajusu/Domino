...
func _ready():
	...
	var base_pack = mazo_original["*StampName* Pack"]
	
	
	for stamp_key in Diccionarios.stamps.keys():
		var nombre = stamp_key    # ej: "Fire", "Gold", etc.
		var nuevo_key = nombre + " Pack"
		
		keys_stamps_paks.append(nuevo_key)
		
		var nuevo = base_pack.duplicate(true)
		
		nuevo["titulo"] = Diccionarios.stamps[nombre].titulo + " Pack"
		nuevo["descripcion"] = "<#296e8f>Gives the " + nombre + "; can be used on special or normal domino.\n<#296e8f>(only <#296e8f>1)"
		
		mazo_original[nuevo_key] = nuevo
...
var amuletos_gratis_restantes = 0
...
func draw_stamps(n):
	var amuletos_tenidos = get_tree().current_scene.amuletos_tenidos
	
	mazo_actual.erase("*StampName* Pack")
	
	var mazo_actual_aux = mazo_actual.duplicate()
	
	if Global.stats.poder_repetir_amuletos:
		mazo_actual = Global.stamps.duplicate()
	...
	for _i in range(n):
		...
		for nombre in keys:
			var data = mazo_actual[nombre]
			var chance = data["chance"]
			
			if nombre in amuletos_tenidos and !Global.stats.poder_repetir_amuletos:
				chance = 0
			
			# Bloqueo por nivel
			if nivel_max < data["nivel_desbloqueo"]:
				chance = 0
			
			chances[nombre] = chance
			total_chance += chance
		...
		if keys_stamps_paks.has(nombre_seleccionado):
			var nombre_sin_pack = nombre_seleccionado
			nombre_sin_pack = nombre_sin_pack.substr(0, nombre_sin_pack.length() - " Pack".length())
			s.get_node("Sprite/stamp").region_rect.position = Diccionarios.stamps[nombre_sin_pack].position
		...
		print("amuletos_gratis_restantes   ", amuletos_gratis_restantes)
		
		if amuletos_gratis_restantes != 0:
			s.gratis = true
			costo_label.bbcode_text  = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode("<#b1911a>0")) + "[color=#b1911a]" + Global.prefix_plata
			amuletos_gratis_restantes -= 1
		else:
	...
	if Global.stats.poder_repetir_amuletos:
		mazo_actual = mazo_actual_aux.duplicate()
...
func _physics_process(_delta):
		...
		if get_parent().get_parent().get_parent().baraja_activa != "usando_amuleto":
