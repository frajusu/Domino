...
func _ready():
	...
	mazo_hyper = Global.dominos.duplicate()
	mazo_hyper.merge(Global.dominos_especiales, true)
...
#var domino = preload("res://Scenas/Specials_tienda_seleccion.tscn")
#var normal = preload("res://Scenas/Normals_tienda_seleccion.tscn")

func _draw_specials(n, sacar_stamps = false):
	...
	for _i in range(n):
		...
		var nombre = nombre_seleccionado
		var _datos = mazo_actual[nombre_seleccionado]
		...
		s.nombre = nombre
		s.name = nombre
		s.yo = mazo_actual[nombre].duplicate()
		add_child(s)
		...
		var nombre_corregido = Global.quitar_t_extra(nombre)

		if get_tree().root.get_node("Game/Shaders").has_node(nombre_corregido):
			var asd = get_tree().root.get_node("Game/Shaders").get_node(nombre_corregido).duplicate()
			s.get_node("Sprite").add_child(asd)
			asd.z_index = 0

		if nombre_corregido == "Sniper":
		...
		titulo_label.bbcode_text = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(mazo_actual[nombre]["titulo"]))
		desc_label.bbcode_text   = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(mazo_actual[nombre]["descripcion"]))
		costo_label.bbcode_text  = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(mazo_actual[nombre]["plata"])) +"[color=#b1911a]"+ Global.prefix_plata
		...
		if sacar_stamps and rng.randf() <= 1.1:
			s.yo.stamps = []
			var stamp_name = _get_random_stamp(rng, 3)
...
func draw_cards(n, sacar_stamps := false, descripcion := "", normales := 1, para_poner_stamps = false):
	var current_cards = get_children().size()
	if current_cards >= Global.stats.max_cards_in_hand:
		return
	...
	# ============================================================
	#   🔹 SI normales == false → USAR SOLO ESPECIALES (tu funcion 1)
	# ============================================================
	if not para_poner_stamps:
		_draw_specials(n, sacar_stamps)
		return
	
	if para_poner_stamps:
		mazo_hyper = get_tree().root.get_node("Game/Viewport/Baraja").mazo_original.duplicate()
		mazo_hyper.merge(get_tree().root.get_node("Game/Viewport/Zona_de_specials/Baraja_S").mazo_original.duplicate(), true)
	
	# ============================================================
	#   🔹 SI normales == true → 50/50 normal o especial
	# ============================================================
	for _i in range(n):
		if keys.size() == 0:
			break
		...
		keys = mazo_hyper.keys()  # refrescar
		
		var chance = rng.randf()   # 0.0 a 1.0
		
		if normales != 2:
			if chance <= 0.5 or normales == 1:
				# ------------ ESPECIAL ---------------
				_draw_one_special(rng, sacar_stamps, normales)
			else:
				# ------------ NORMAL ----------------
				_draw_one_normal(rng, sacar_stamps, descripcion, normales)
		else:
			_draw_one_normal(rng, sacar_stamps, descripcion, normales)
...
func _draw_one_special(rng, sacar_stamps, normales = 1, keys = get_tree().root.get_node("Game/Viewport/Zona_de_specials/Baraja_S").mazo_original.keys()):
	# chances
	rng.randomize()
	
	# --- Filtrar solo los que no estan llenos ---
	var candidatos = []
	for nombre in keys:
		if mazo_hyper.keys().has(nombre) and mazo_hyper[nombre].stamps.size() < Global.stats.maximo_stampas:
			candidatos.append(nombre)
	
	# Si NO hay candidatos, sortear un normal y salir
	if candidatos.size() == 0:
		_draw_one_normal(rng, sacar_stamps, "", normales)
		return
	
	# --- Calcular total_chance solo con los candidatos ---
	var total_chance = 0
	for nombre in candidatos:
		total_chance += mazo_hyper[nombre]["chance"]
	
	var roll = rng.randf_range(0, total_chance)
	var acumulado = 0
	var nombre = candidatos[0]
	
	for n in candidatos:
		acumulado += mazo_hyper[n]["chance"]
		if roll <= acumulado:
			nombre = n
			break
	...
	if sacar_stamps and rng.randf() <= 1.1:
		s.yo.stamps = []
		var stamp_name = _get_random_stamp(rng, normales)
		var pos_x = rng.randi_range(-10, 10)
		var pos_y = rng.randi_range(-30, 30)
		s.yo.stamps.append([stamp_name, Vector2(pos_x, pos_y)])
		s.poner_stamps()
...
func _draw_one_normal(rng, sacar_stamps, descripcion, normales=1, keys = Global.dominos.keys()):
	# Filtrar solo los que NO estan llenos y existen en mazo_hyper
	var candidatos = []
	for nombre in keys:
		if mazo_hyper.keys().has(nombre) and mazo_hyper[nombre].stamps.size() < Global.stats.maximo_stampas:
			candidatos.append(nombre)
	
	# Si NO hay candidatos validos, no podemos sortear ninguno
	if candidatos.size() == 0:
		Global.mostrar_alert_error("NO QUEDA NINGUN DOMINO DISPONIBLE (todos llenos)")
		return
	
	var nombre = candidatos[rng.randi_range(0, candidatos.size() - 1)]
	
	var data = mazo_hyper[nombre]
	
	var s = normal.instance()
	s.set_meta("base_pos", s.get_node("Sprite").position)
	s.nombre = nombre
	s.name = nombre
	...
	if sacar_stamps and rng.randf() <= 0.10:
		s.yo.stamps = []
		var st = _get_random_stamp(rng, normales)
		s.yo.stamps.append([st, Vector2(rng.randi_range(-10,10), rng.randi_range(-30,30))])
		s.poner_stamps()
	
	mazo_hyper.erase(nombre)


# ----------------------------------------------------------
#  Funcion auxiliar para elegir stamp segun chance
# ----------------------------------------------------------
func _get_random_stamp(rng, normales = 1):
	var dic = Diccionarios.stamps
	var keys = dic.keys()

	# si normales == 3, removemos "Double Stamp"
	if normales == 3:
		var nuevas_keys = []
		for k in keys:
			if k != "Double Stamp":
...
func _physics_process(_delta):
	if dragging != null and get_parent().get_parent().estacion_actual != "tienda":
		dragging = null
	...
	if get_parent().get_parent().estacion_actual == "tienda" and get_parent().get_parent().baraja_activa == "usando_amuleto":
		if event is InputEventMouseButton:
			...
			if event.button_index == BUTTON_LEFT:
				if event.pressed:
					for s in get_children():
						if s != arrastrado and s.arrastrable:
