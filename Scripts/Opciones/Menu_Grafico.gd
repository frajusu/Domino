...
func _physics_process(_delta):
	if visible:
		...
		var botones_stats = {
			...
			$Botones.get_node("<_wrap") : Global.grafico.wrap / 0.1,
		}
				...
				if Input.is_action_pressed("click"):
					...
					if botonHover[boton1.name]:
						...
						match boton1.name:
							...
							"<_wrap":
								Global.grafico.wrap = clamp(Global.grafico.wrap - paso_wrap, 0.0, 0.1)
							">_wrap":
								Global.grafico.wrap = clamp(Global.grafico.wrap + paso_wrap, 0.0, 0.1)
