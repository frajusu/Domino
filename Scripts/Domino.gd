...
func crear_puntitos():
	...
	if yo.stamps != []:
		for i in yo.stamps:
			...
			instancia.get_node("Descripcion/Label").bbcode_text = "[center]%s[/center]" % parsear_colores_bbcode(Diccionarios.stamps[i[0]]["descripcion_corta"])
			instancia.get_node("Titulo/Label").bbcode_text      = "[center]%s[/center]" % parsear_colores_bbcode(Diccionarios.stamps[i[0]]["titulo"])
