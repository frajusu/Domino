...
func actualizar_sombra():
	...
	if get_parent().get_parent().get_parent().dragging == self:
...
func crear_puntitos():
	...
	if domino_data["tipo"] == "normal":
		...
		$Parte_de_costado.region_rect = Rect2(Vector2(29, 0), Vector2(6, 64))
		pass
...
func _process(delta):
	...
	if valor_dragin != get_parent().get_parent().get_parent().dragging:
		update_focus()
		valor_dragin = get_parent().get_parent().get_parent().dragging
...
func update_focus():
	...
	for d in get_tree().get_nodes_in_group("dominos"):
		if !get_parent().get_parent().get_parent().dragging:
			if d == leftmost:
