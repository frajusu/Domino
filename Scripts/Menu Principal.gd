...
var menu_actual = "transision_a_default"
...
func _physics_process(_delta):
	if menu_actual == "transision_a_default":
		...
		$Viewport/Menu/Options.position.y = lerp($Viewport/Menu/Options.position.y, 54, 0.1)
		$Viewport/Menu/Quit.position.y = lerp($Viewport/Menu/Quit.position.y, 54, 0.1)
		...
		for i in range(menu.get_child_count()):
			...
			if dentro:
				...
				if Input.is_action_pressed("click"):
					...
					match boton.name:
						"Quit":
							boton.global_position = Vector2(180, 68)
					...
					if botonHover[boton.name]:
						...
						if boton.name == "Play":
							...
							boton.global_position = Vector2(-142, 63)
