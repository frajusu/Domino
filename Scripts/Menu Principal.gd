...
onready var botonPosision =  {  "Play" :     {"global_position" : Vector2(-157, 83)},
								...
								"Charms" :   {"global_position" : Vector2(67, 83)},
}
...
var nivel_max_ultimo = Global.leer_save("Partida", "Nivel_Maximo_actual", 0)
...
func _ready():
	...
	randomize()
	tiempo_restante = 13.0
	
	if get_tree().get_nodes_in_group("pantalla_carga").size() == 0:
		menu_actual = "transision_a_default"
	
	$Viewport/Camera2D/uso_amuleto_anim.get_node("Sprite/Money").visible = true
	...
	$Black.visible = true
	$Menu_amuletos.visible = true
	
	if Global.desbloquear or true:
		yield(get_tree().create_timer(0.6), "timeout")
		for amuleto in Diccionarios.amuletos.keys():
			var data = Diccionarios.amuletos[amuleto]
			if nivel_max == null or nivel_max_ultimo == null:
				nivel_max_ultimo = 1
				nivel_max = 1
			
			var desbloqueado =  data["nivel_desbloqueo"] > nivel_max and data["nivel_desbloqueo"] <= nivel_max_ultimo
			
			if desbloqueado:
				yield(desbloquear_amuleto(amuleto), "completed")
	
	var nivel_maximo_actual = Global.leer_save("Partida", "Nivel_Maximo_actual", 0)
	var nivel_maximo =        Global.leer_save("Partida", "Nivel_Maximo", 0)
	
	if nivel_maximo_actual > nivel_maximo:
		Global.guardar_save("Partida", "Nivel_Maximo", nivel_maximo_actual)


func desbloquear_amuleto(_amuleto) -> void:
	var nodo = $Viewport/Camera2D/uso_amuleto_anim
	nodo.get_node("Sprite").region_rect.position = Diccionarios.amuletos[_amuleto].position
	nodo.get_node("AnimationPlayer").play("anim")
	
	yield(nodo.get_node("AnimationPlayer"), "animation_finished")
...
onready var player = $Viewport/AudioStreamPlayer2D
...
func _physics_process(delta):
	...
	if tiempo_restante <= 0:
		$BackGround/Genie/AnimationPlayer.play("mover")
	
		if primera_vez:
			primera_vez = false
			tiempo_restante = rand_range(20.0, 40.0)
		else:
			tiempo_restante = rand_range(20.0, 40.0)
	...
	if menu_actual == "transision_a_default":
		...
		$Viewport/Menu/Charms.position.y = $Viewport/Menu/Play.position.y
		...
		for i in range(menu.get_child_count()):
			...
			if dentro:
				...
				if Input.is_action_pressed("click"):
					...
					match boton.name:
						"Quit":
							boton.position = Vector2(163.143, 58)
					...
					if botonHover[boton.name]:
						...
						if boton.name == "Charms":
							Global.sonido_boton()
							$Menu_amuletos.mostrarse()
							get_tree().paused = true
...
#						boton.get_node("Play_sprites").material.set_shader_param("x_rot", -diferencia_y / 2)
	...
	if menu_actual == "select_partida":
				...
				if Input.is_action_pressed("click"):
					...
					if botonHover[boton.name]:
						...
						if boton.name == "NEW":
							...
							if Global.tutorial:
								Cargador.goto_scene("res://Scenas/cinematicas/menos_info.tscn")
							else:
								Cargador.goto_scene("res://Scenas/cinematicas/intro.tscn")
		...
		if true:
			var boton = $"Seleccionar_partida/1/b_bgt"
			var pos = get_global_mouse_position()
			var diferencia = boton.rect_size
			var dentro_x = pos.x > boton.rect_global_position.x and pos.x < boton.rect_global_position.x + diferencia.x +7
			var dentro_y = pos.y > boton.rect_global_position.y and pos.y < boton.rect_global_position.y + diferencia.y
			var dentro = dentro_x and dentro_y
			
			if Global.tutorial: boton.get_node("bit").bbcode_text = "[center][wave amp=13 freq=1]On [/wave][/center]" 
			else: boton.get_node("bit").bbcode_text = "[center][wave amp=13 freq=1]Off [/wave][/center]"
		...
		var boton = $"Seleccionar_partida/1/b_end"
		var pos = get_global_mouse_position()
		var diferencia = boton.rect_size
		var dentro_x = pos.x > boton.rect_global_position.x and pos.x < boton.rect_global_position.x + diferencia.x +7
		var dentro_y = pos.y > boton.rect_global_position.y and pos.y < boton.rect_global_position.y + diferencia.y
		...
		if Global.endless: boton.get_node("bit").bbcode_text = "[center][wave amp=13 freq=1]On [/wave][/center]" 
		else: boton.get_node("bit").bbcode_text = "[center][wave amp=13 freq=1]Off [/wave][/center]"
		...
		if dentro:
			...
			if Input.is_action_just_pressed("click"):
				...
				Global.guardar_save("Misc", "endless", Global.endless)
				$Black.shake_reroll()
