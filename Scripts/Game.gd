		...
		for boton in botones:
			...
			if boton.name == "Skip":
				...
				if dragging:
					...
					boton.rotation = lerp(boton.rotation, rot_objetivo, 0.15)
				else:
					if dentro and boton.visible:
						if domino_a_conseguir:
							var tipo = ""
					...
					boton.rotation = lerp(boton.rotation, 0.0, 0.15)
...
#	charm.get_node("tope/Pixe").visible = false
	...
	for i in $Barajas_seleccion.get_children():
		if i == $Barajas_seleccion/stampa_a_colocar:
			continue
...
func colocar_stampa(stampa, _a_donde):
	...
	if _a_donde == "all":
		$Barajas_seleccion/specials.draw_cards(5, false, "", true, true)
