						...
						"Buy":
							...
							if domino_a_comprar:
								if domino_a_comprar.get_parent().name == "Baraja_normales":
									...
									if plata >=  int($Viewport/Zona_de_tienda/Baraja_stamps.mazo_original[domino_a_comprar.name]["plata"].strip_edges().replace("<#b1911a>", "")):
										plata -= int($Viewport/Zona_de_tienda/Baraja_stamps.mazo_original[domino_a_comprar.name]["plata"].strip_edges().replace("<#b1911a>", ""))
										...
										if !$Viewport/Zona_de_tienda/Baraja_stamps.mazo_original[domino_a_comprar.name]["usable"]:
									...
									Global.stamps[domino_a_comprar.name] = (Diccionarios.amuletos[domino_a_comprar.name])
...
func _on_aparecer_completo(estacion) -> void:
	...
	if estacion == "seleccion_nivel":
		...
		print("asd          ", amuletos_tenidos)
		print("asd          ", plata)
		print("asd          ", Global.leer_save("Partida","Money"))
...
func invocar_cosas(amuleto):
	...
	match amuleto: #   PARA LOS QUE HACEN ALGO MUY ESPECIFICO
		"Color Infusion Pack":
			...
			$Barajas_seleccion/normals.draw_cards(Diccionarios.amuletos[amuleto].cuantos, Diccionarios.amuletos[amuleto].stamps, ["naranja", "rosa", "azul", "verde"])
			return
		...
		"Chainmaker Pack":
			...
			$Viewport/Open_charm/tope/seleccion/dominos.visible = true
			$Viewport/Open_charm/tope/seleccion/NEW.bbcode_text = "[center][wave amp=13 freq=1]\n Choose... "+ str(Diccionarios.amuletos[amuleto].cuantos_agarrables) +" [/wave][/center]"
			$Barajas_seleccion/normals.draw_cards(Diccionarios.amuletos[amuleto].cuantos, Diccionarios.amuletos[amuleto].stamps, [mayor_color])
			return
...
#	charm.get_node("tope/Pixe").visible = false
	...
	for i in $Barajas_seleccion.get_children():
		if i == $Barajas_seleccion/stampa_a_colocar or i == $Barajas_seleccion/stampa_mouse:
			continue
...
func insertar_stampa(domino, stampa = stampa_a_colocar):
	...
	domino.yo.stamps.append([stampa, get_global_mouse_position()-domino.global_position])
	
	domino.poner_stamps()
	
	match domino.get_script().resource_name:
