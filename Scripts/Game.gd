...
func _empezar_animacion_de_bajar():
	...
	var pixeles_bajar := 100
	var duracion := 0.4
	
	var tween = create_tween().set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	
	# Todo PARALLEL desde el principio
	tween.parallel().tween_property(cuerpo, "position:y", cuerpo.position.y + pixeles_bajar, duracion)
	tween.parallel().tween_property(godray, "position",   Vector2(5, cuerpo.position.y + pixeles_bajar + 10), duracion)
	tween.parallel().tween_property(sprite, "position:y", sprite.position.y + pixeles_bajar, duracion)
	
	tween.parallel().tween_property(cuerpo, "scale", cuerpo.scale * 2.0, duracion)
	tween.parallel().tween_property(godray, "scale", Vector2(60, -60.0), duracion)
	tween.parallel().tween_property(sprite, "scale", sprite.scale * 2.0, duracion)
