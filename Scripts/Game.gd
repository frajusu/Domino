...
func _physics_process(_delta):
	if charm_shaking == 1 or charm_shaking == 2:
		...
		charm.rotation = sin(charm_shake_time * charm_shake_speed) * deg2rad(charm_shake_strength)
	...
	$Viewport/Camera2D/camara.visible = Global.overlay
	if !Global.overlay_default:
		$Viewport/Camera2D/camara.material.set_shader_param("vignette_opacity", Global.vignete_opacity)
		$Viewport/Camera2D/camara.material.set_shader_param("warp_amount", Global.wrap)
		$Viewport/Camera2D/camara.material.set_shader_param("static_noise_intensity", Global.noise)
		$Viewport/Camera2D/camara.material.set_shader_param("posterize_colors", Global.reducir_colores)
	else:
		$Viewport/Camera2D/camara.material.set_shader_param("vignette_opacity", 0.262)
		$Viewport/Camera2D/camara.material.set_shader_param("warp_amount", 0.083)
		$Viewport/Camera2D/camara.material.set_shader_param("static_noise_intensity", 0.021)
		$Viewport/Camera2D/camara.material.set_shader_param("posterize_colors", true)
...
func _on_aparecer_completo(estacion) -> void:
	...
	if estacion == "tienda":
		...
		$Viewport/Zona_de_tienda/Baraja_normales.draw_cards(Global.stats.max_cards_normal_in_store, Global.dominos_en_shop_tener_stamps or Global.normales_pueden_tener_stamps)
		
		$Viewport/Zona_de_tienda/Baraja_specials.mazo_actual = $Viewport/Zona_de_tienda/Baraja_specials.mazo_original.duplicate()
		$Viewport/Zona_de_tienda/Baraja_specials.draw_cards(Global.stats.max_cards_specials_in_store, Global.dominos_en_shop_tener_stamps or Global.specials_pueden_tener_stamps)
...
func crear_amuleto(amuleto):
	...
	costo_label.bbcode_text  = "[center]%s[/center]" % grid.parsear_colores_bbcode(data["venta"]) + "[color=#b1911a]" + Global.prefix_plata
...
#	var sprite = get_node(base + "AnimatedSprite")
	
#	var pixeles_bajar := 60
#	var duracion := 0.4
	
	# 👉 Tween viejo (el que SI tiene tween_all_completed)
#	var tween := Tween.new()
#	add_child(tween)
...
#
#	# Propiedades
#	var _a = tween.interpolate_property(cuerpo, "position:y",
#		cuerpo.position.y, cuerpo.position.y + pixeles_bajar,
#		duracion, Tween.TRANS_BACK, Tween.EASE_OUT)
#
#	_a = tween.interpolate_property(godray, "position",
#		godray.position, Vector2(5, cuerpo.position.y + pixeles_bajar + 10),
#		duracion, Tween.TRANS_BACK, Tween.EASE_OUT)
#
#	_a = tween.interpolate_property(sprite, "position:y",
#		sprite.position.y, sprite.position.y + pixeles_bajar,
#		duracion, Tween.TRANS_BACK, Tween.EASE_OUT)
#
#	# Escalas
#	_a = tween.interpolate_property(cuerpo, "scale",
#		cuerpo.scale, cuerpo.scale * 2.0,
#		duracion, Tween.TRANS_BACK, Tween.EASE_OUT)
#
#	_a = tween.interpolate_property(godray, "scale",
#		godray.scale, Vector2(60, -60.0),
#		duracion, Tween.TRANS_BACK, Tween.EASE_OUT)
#
#	_a = tween.interpolate_property(sprite, "scale",
#		sprite.scale, sprite.scale * 2.0,
#		duracion, Tween.TRANS_BACK, Tween.EASE_OUT)
#
#	_a = tween.start()
