...
func _physics_process(_delta):
	...
	if Input.is_action_pressed("subir_puntos") and estacion_actual == "nivel" and !bandera_nivel and baraja_activa == "baraja":
		sumar_puntos(5)
	...
	if baraja_activa == "Deck" or baraja_activa == "Deck1":
		if baraja_activa == "Deck":
			$Viewport/Menu_info/Rojo.layout_width = 950
			$Viewport/Menu_info/Azul.layout_width = 950
			$Viewport/Menu_info/Verde.layout_width = 950
			$Viewport/Menu_info/Amarillo.layout_width = 950
			
			$Viewport/Menu_info/Rojo.draw_cards("rojo")
			$Viewport/Menu_info/Azul.draw_cards("azul")
			$Viewport/Menu_info/Verde.draw_cards("verde")
			$Viewport/Menu_info/Amarillo.draw_cards("amarillo")
			
			baraja_activa = "Deck1"
		
		if Input.is_action_just_pressed("ui_cancel"):
			for i in $Viewport/Menu_info/Rojo.get_children():
				i.free()
			
			for i in $Viewport/Menu_info/Azul.get_children():
				i.free()
			
			for i in $Viewport/Menu_info/Verde.get_children():
				i.free()
			
			for i in $Viewport/Menu_info/Amarillo.get_children():
				i.free()
					...
					match boton.name:
						"Buy":
							...
							if domino_a_comprar:
								...
								#print(domino_a_comprar.name)
								
								if Global.dominos.has(domino_a_comprar.name):
									var nuevo_nombre = domino_a_comprar.name
									
									# mientras ya exista una carta con ese nombre en el mazo, agregarle "t"
									...
									$Viewport/Menu_info/Rojo.mazo_original = $Viewport/Baraja.mazo_original.duplicate()
									$Viewport/Menu_info/Azul.mazo_original = $Viewport/Baraja.mazo_original.duplicate()
									$Viewport/Menu_info/Verde.mazo_original = $Viewport/Baraja.mazo_original.duplicate()
									$Viewport/Menu_info/Amarillo.mazo_original = $Viewport/Baraja.mazo_original.duplicate()
