...
func _set_puntos_temp(v):
	...
	var texto_numeros = $Viewport/Zona_de_cosas/Points_A.bbcode_text.replace("[wave amp=50 freq=2]", "").replace("[/wave]", "")
	var texto_numeros_length = texto_numeros.length()-2
...
func _physics_process(_delta):
	if Input.is_action_pressed("subir_puntos"):
		sumar_puntos(1000)
