				...
				match boton.name:
					"Delete":
						...
						if domino_a_borrar:
							...
							boton.get_node("Play_sprites/AnimationPlayer").play("achicarse")
...
func desaparecer(destino: float) -> void:
	...
	_a = tween.interpolate_property(
		...
		get_node("Viewport/Zona_de_interfaz/Select/1").position.y, 327,
		1, Tween.TRANS_CUBIC, Tween.EASE_IN_OUT
	)
	_a = tween.interpolate_property(
		get_node("Viewport/Zona_de_interfaz/Select/Menu/1"), "position:x",
...
func aparecer() -> void:
	...
	_a = tween.interpolate_property(
		$Viewport/Zona_de_cosas, "position:y", $Viewport/Zona_de_cosas.position.y, -76,
		1, Tween.TRANS_BACK, Tween.EASE_OUT
	)
	_a = tween.interpolate_property(
