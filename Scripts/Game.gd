...
#			salir_de_usable()
	...
	if estacion_actual == "transicion":
		if Global.mover_camara:
			...
			$Viewport/TiendaBG.position = (((posision-$Viewport/TiendaBG.position)/40)) + Vector2(estaciones[estacion_aux], 0)
			...
			$Viewport/TiendaBG.position = Vector2(estaciones[estacion_aux], 0)
			get_node("Viewport/Cartelitos").position.x = estaciones[estacion_actual]
	else:
		if Global.mover_camara:
			...
			$Viewport/TiendaBG.position = (((posision-$Viewport/TiendaBG.position)/40)) + Vector2(estaciones[estacion_actual], 0)
			get_node("Viewport/Cartelitos").position.x = estaciones[estacion_actual]
		else:
			camara.position = Vector2(estaciones[estacion_actual], 0)
			$Viewport/GameBG.position = Vector2(estaciones[estacion_actual], 0)
			$Viewport/TiendaBG.position = Vector2(estaciones[estacion_actual], 0)
			get_node("Viewport/Cartelitos").position.x = estaciones[estacion_actual]
	...
	if estacion_actual == "seleccion_nivel" and !bandera_go_seleccion:
		if $Viewport/TiendaBG.modulate.a8 > 0:
			$Viewport/TiendaBG.modulate.a8 -= 10
			
			$"Viewport/1/1/Draws/NinePatchRect".material.set_shader_param("color_target", Color("#2f5f4a"))
	...
	if estacion_actual == "tienda":
		if $Viewport/TiendaBG.modulate.a8 < 256:
			$Viewport/TiendaBG.modulate.a8 += 10
			
			if Global.usar_color_tienda_en_menus:
				$"Viewport/1/1/Draws/NinePatchRect".material.set_shader_param("color_target", Color("#908459"))
	...
	if estacion_actual == "tienda" and !bandera_go_tienda and baraja_activa != "usando_amuleto":
		if Global.usar_color_tienda_en_menus:
			$Viewport/Zona_de_interfaz/Tienda/Zona_de_specials/Specials1.material.set_shader_param("to_color", Color("#cab26b"))
		else:
			$Viewport/Zona_de_interfaz/Tienda/Zona_de_specials/Specials1.material.set_shader_param("to_color", Color("#214336"))
				...
				if dentro:
					if !Global.bandera_mouse:
						...
						if (baraja_activa == "baraja") or baraja_activa == "baraja_especial":
							if Global.usar_color_tienda_en_menus:
								$Viewport/Zona_de_interfaz/Tienda/Zona_de_specials/Specials1.material.set_shader_param("to_color", Color("#fff6bc"))
							else:
		...
		if Global.usar_color_tienda_en_menus:
			$Viewport/Zona_de_interfaz/Tienda/Zona_de_amuletos/Specials1.material.set_shader_param("to_color", Color("#cab26b"))
		else:
			...
			if dentro:
				...
				if dentro:
					if !Global.bandera_mouse:
						...
						if (baraja_activa == "baraja") or baraja_activa == "baraja_amuletos":
							if Global.usar_color_tienda_en_menus:
								$Viewport/Zona_de_interfaz/Tienda/Zona_de_amuletos/Specials1.material.set_shader_param("to_color", Color("#fff6bc"))
							else:
						...
						"Buy":
							...
							if domino_a_comprar:
								...
								if domino_a_comprar.get_parent().name == "Baraja_normales":
									...
									var precio_str = Global.stamps[nombre_domino_a_comprar]["plata"]
									var precio = int(precio_str.replace("<#b1911a>", ""))  # saco el tag
									...
									if plata >=  int(precio_bbcode.strip_edges().replace("<#b1911a>", "")) and amuletos_tenidos.size() < Global.stats.max_amuletos:
										plata -= int(precio_bbcode.strip_edges().replace("<#b1911a>", ""))
										...
										if !Global.stamps[nombre_domino_a_comprar]["usable"]:
											amuletos_tenidos.append(nombre_domino_a_comprar)
											
											crear_amuleto(nombre_domino_a_comprar)
											
											
											var amuleto_a_afectar = Diccionarios.amuletos[nombre_domino_a_comprar]
									...
									plata += int(Global.stamps[nombre_domino_a_comprar]["venta"].strip_edges().replace("<#b1911a>", ""))
									...
									amuletos_tenidos.erase(nombre_domino_a_comprar)
									
									aplicar_efecto(nombre_domino_a_comprar)
									
									$Viewport/Zona_de_interfaz/xd/Zona_de_amuletos/Baraja_S/ScrollContainer/MarginContainer/GridContainer.get_node(nombre_domino_a_comprar).free()
									domino_a_comprar.queue_free()
									Global.usar_offset = true
									domino_a_comprar = null
...
func insertar_stampa(domino, stampa = stampa_a_colocar):
	...
	if domino.yo.titulo == "<#e0483e>Stamp <#e0483e>Master":
		if domino.yo.stamps.size() == 0:
			domino.yo.stamps.append([stampa, ((get_global_mouse_position()-domino.global_position)*0.7)+Vector2(0, -10)])
			domino.poner_stamps()
		pass
...
func aplicar_efecto(_amuleto):
	for i in EFECTOS_TEMPORALES_AMULETOS.duplicate():
		...
		if amuletos_tenidos.has(_amuleto):
			# aplicar
			if i[3]:
				match operador:
					"x":
