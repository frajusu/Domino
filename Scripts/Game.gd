								...
								if domino_a_comprar.get_parent().name == "Baraja_normales":
									...
									if plata >=  int(precio_bbcode.strip_edges().replace("<#b1911a>", "")) and amuletos_tenidos.size() < Global.stats.max_amuletos - resta:
										...
										for k in range(0, amuletos_tenidos.size()):
											...
											var costo_label  = amuleto_nodo.get_node("Descripcion/MarginContainer/Costo/Label")
											...
											var costo_label1  = amuleto_nodo1.get_node("Descripcion/MarginContainer/Costo/Label")
											...
											costo_label1.bbcode_text  = costo_label.bbcode_text
...
func crear_amuleto(amuleto, array_valores = {}):
	...
	s.valores_diccionario = array_valores.duplicate()
	...
	s1.valores_diccionario = array_valores.duplicate()
