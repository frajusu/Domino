...
func plata_por_nivel(nivel: int) -> int:
	...
	var base := 10              # plata base del nivel 1
	var crecimiento := 1.15     # factor de crecimiento (igual que en puntos)
	var variacion := 3          # variacion fija por nivel (mas chica para la plata)
	
	var plata1 = base * pow(crecimiento, nivel - 1)
	
	# variacion pseudoaleatoria en base al nivel
	var pseudo_rand = int(hash(nivel) % variacion)
	plata1 += pseudo_rand
...
func _on_aparecer_completo(estacion) -> void:
	if estacion == "nivel":
		...
		$Viewport/Baraja.mazo_original = Global.dominos.duplicate()
	...
	if estacion == "tienda":
		$Viewport/Zona_de_tienda/Baraja_normales.mazo_original = Global.dominos.duplicate()
		...
		$Viewport/Zona_de_tienda/Baraja_specials.mazo_original = Global.dominos_especiales.duplicate()
		...
		$Viewport/Zona_de_tienda/Baraja_stamps.mazo_original = Global.stamps.duplicate()
...
func actualizar_escenario_seleccion_nivel():
	...
	get_node("Viewport/1/1/Money/Monedas").bbcode_text = "[center][wave amp=50 freq=2]\n"+str(plata)+prefix_plata+"\n[/wave]"
