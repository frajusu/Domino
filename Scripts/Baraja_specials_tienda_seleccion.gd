...
func draw_cards(n, sacar_stamps := false, descripcion := "", normales := false, para_poner_stamps = false):
	var current_cards = get_children().size()
	if current_cards >= Global.stats.max_cards_in_hand:
		return
	...
	for _i in range(n):
		...
		if chance <= 0.5 or normales == false:
			# ------------ ESPECIAL ---------------
...
func _draw_one_special(rng, sacar_stamps, keys = get_tree().root.get_node("Game/Viewport/Zona_de_specials/Baraja_S").mazo_original.keys()):
	...
	var total_chance = 0
	for nombre in keys:
		if !mazo_hyper.keys().has(nombre): continue
...
func _draw_one_normal(rng, sacar_stamps, descripcion, keys = Global.dominos.keys()):
	var nombre = keys[rng.randi_range(0, keys.size() - 1)]
	
	while !mazo_hyper.keys().has(nombre):
		rng.randomize()
		nombre = keys[rng.randi_range(0, keys.size() - 1)]
