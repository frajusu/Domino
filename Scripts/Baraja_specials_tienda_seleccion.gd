	...
	for _i in range(n):
		...
		if normales != 2:
			if chance <= 0.5 or normales == 1:
				# ------------ ESPECIAL ---------------
				_draw_one_special(rng, sacar_stamps)
			else:
				# ------------ NORMAL ----------------
				_draw_one_normal(rng, sacar_stamps, descripcion)
		else:
			_draw_one_normal(rng, sacar_stamps, descripcion)
...
func _draw_one_special(rng, sacar_stamps, keys = get_tree().root.get_node("Game/Viewport/Zona_de_specials/Baraja_S").mazo_original.keys()):
	# chances
	rng.randomize()
	...
	if candidatos.size() == 0:
		_draw_one_normal(rng, sacar_stamps, "")
		return
...
func _draw_one_normal(rng, sacar_stamps, descripcion, keys = Global.dominos.keys()):
