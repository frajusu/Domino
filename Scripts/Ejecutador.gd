...
var delay_por_domino := 2 # segundos entre animaciones
...
onready var barras := get_tree().root.get_node("Game/Barras/barras")


func _process(delta):
	puntos_shader_max = barras.get_parent().get_parent().puntos_por_nivel(barras.get_parent().get_parent().nivel_actual)
	barras.material.set_shader_param("progress1", lerp(barras.material.get_shader_param("progress1"), valor_actual, delta * 8))
...
func _call_next_step(i):
	...
	var puntos_actuales = paso["puntos_acumulados"]
	
	valor_actual = clamp(puntos_actuales / puntos_shader_max * 0.8, 0.0, 0.8)
	print("Shader =", valor_actual, " | Domino:", paso["domino"].name, " | Puntos:", puntos_actuales)
	
	var _a = get_tree().create_timer(delay_por_domino).connect("timeout", self, "_call_next_step", [i + 1])
