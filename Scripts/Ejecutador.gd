...
var delay_por_domino := 1 # segundos entre animaciones
...
func _process(delta):
	if get_tree().root.has_node("Game"):
		puntos_shader_max = barras.get_parent().get_parent().puntos_por_nivel(barras.get_parent().get_parent().nivel_actual)
...
# Suponiendo que este nodo de animacion esta accesible, por ejemplo:
var nodo_anim
...
func _call_next_step(i):
	...
	# ✅ Sumamos al total global mostrado
	...
	# Mover el nodo de animacion sobre el domino actual
	nodo_anim.global_position = domino.global_position + Vector2(0, -30)
	
	# ⚡ Primero mostrar el multiplicador (si hay)
	if paso["mult"] != 1:
		nodo_anim.mostrar_tipo("mult", paso["mult"], 2.0, [20, 50, 40], 0.4)
		nodo_anim.get_parent().shake_reroll()
		yield(get_tree().create_timer(0.6), "timeout") # delay antes del puntaje
	
	# 🎯 Luego mostrar el puntaje
