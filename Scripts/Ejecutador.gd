...
func _call_next_step(i):
	...
	if i >= secuencia_puntaje.size():
		...
		for i in get_tree().root.get_node("Game").EFECTOS_TEMPORALES.duplicate():
			...
			if i[2] != 0:
				...
				match operador:
					"x":
						Global.stats[stat] /= valor      # REVERSION CORRECTA
					"+":
						Global.stats[stat] -= valor      # REVERSION CORRECTA
					"-":
						Global.stats[stat] += valor      # REVERSION CORRECTA
	...
	if paso["mult"] != 1:
		nodo_anim.mostrar_tipo("mult", paso["mult"], 2.0, [20, 50, 40], 0.4)
		nodo_anim.get_parent().shake_reroll()
	...
	for i in paso["array_especiales"]:
		...
		if i[1] == "plata":
			get_tree().root.get_node("Game").recibir_plata("Domino ("+str(domino.name)+"): ", i[0])
			pass
...
func revisar_stamps(d):
	...
	for i in d.yo.stamps:
		...
		if i.has("Golden Stamp"):
			array.append([5, "plata"])
		
		if i.has("Silver Stamp"):
			array.append([3, "plata"])
		
		if i.has("Bronze Stamp"):
			array.append([1, "plata"])
		...
		if i.has("Emerald Stamp"):
			...
			array.append([plata_acumulada, "plata"])
		
		if i.has("Obsidian Stamp"):
			get_tree().root.get_node("Game").EFECTOS_TEMPORALES.append(["x2", d.yo.color.to_lower()+"_%", 1])
