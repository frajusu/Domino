	...
	if i + 1 >= secuencia_puntaje.size():
		for k in get_tree().root.get_node("Game").amuletos_tenidos:
			match k:
				...
				"Sea Shell":
					if ultimo["domino"].yo.color.to_lower() == "blue":
...
func revisar_amuletos_por_domino(d):
	...
	for i in get_tree().root.get_node("Game").amuletos_tenidos:
		rng_amuletos.randomize()
		var bool_random = rng_amuletos.randi_range(0, 1) == 1
		match i:
			"Rose Ascendant":
				if bool_random and d.yo.color.to_lower() == "rose": # Orange, Rose, Blue, Green, All
					var baraja = d.get_parent()
					var puntaje = baraja.mazo_original[d.nombre].puntaje
					
					array.append([1, "especial", "Rose Ascendant"])
					
					baraja.mazo_original[d.nombre].puntaje = puntaje+1
					d.yo.puntaje = puntaje+1
			
			"Orange Ascendant":
				if bool_random and d.yo.color.to_lower() == "orange": # Orange, Rose, Blue, Green, All
					var baraja = d.get_parent()
					var puntaje = baraja.mazo_original[d.nombre].puntaje
					
					array.append([1, "especial", "Orange Ascendant"])
					
					baraja.mazo_original[d.nombre].puntaje = puntaje+1
					d.yo.puntaje = puntaje+1
			
			"Blue Ascendant":
				if bool_random and d.yo.color.to_lower() == "blue": # Orange, Rose, Blue, Green, All
					var baraja = d.get_parent()
					var puntaje = baraja.mazo_original[d.nombre].puntaje
					
					array.append([1, "especial", "Blue Ascendant"])
					
					baraja.mazo_original[d.nombre].puntaje = puntaje+1
					d.yo.puntaje = puntaje+1
			
			"Green Ascendant":
				if bool_random and d.yo.color.to_lower() == "green": # Orange, Rose, Blue, Green, All
					var baraja = d.get_parent()
					var puntaje = baraja.mazo_original[d.nombre].puntaje
					
					array.append([1, "especial", "Green Ascendant"])
...
func revisar_amuletos_final():
	...
	for i in get_tree().root.get_node("Game").amuletos_tenidos:
		rng_amuletos_final.randomize()
		var _bool_random = rng_amuletos_final.randi_range(0, 1) == 1
		match i:
			"Giant's Tear Drop":
				if rng_amuletos_final.randi_range(1, 20) == 1:
...
func revisar_amuletos_post_final():
	...
	for i in get_tree().root.get_node("Game").amuletos_tenidos:
		rng_amuletos_post_final.randomize()
		var _bool_random = rng_amuletos_post_final.randi_range(0, 1) == 1
		match i:
			"asdasd":
				pass
