...
func ejecutar_jugada(nodo):
	nivel_actual = get_tree().root.get_node("Game").nivel_actual
	...
	if i >= secuencia_puntaje.size():
		...
		if !repetir_play:
			if get_tree().root.get_node("Game").plays_actuales == 0:
				...
				print(get_tree().root.get_node("Game").puntos_max)
				
				if numero < get_tree().root.get_node("Game").puntos_por_nivel(nivel_actual):
				...
				if numero < get_tree().root.get_node("Game").puntos_por_nivel(nivel_actual):
					...
					if (baraja.mazo_actual.keys().size()) == 0 and baraja.get_child_count() == 0:
						get_tree().root.get_node("Game").perder()
			...
			get_tree().root.get_node("Game/Viewport/Baraja").draw_cards(Global.stats.max_cards_in_hand, true)
