...
func ejecutar_jugada(nodo):
	...
	print("✅ Puntos totales precalculados:", max_puntos_totales)
...
func recorrer_domino(d: Node, mult_global: float, puntaje_extra: int, mult_padre: float, puntos_acumulados: float, indice: int, visitados: Array, en_reversa: bool, comentario = [], ejecuciones = {}):
	...
	secuencia_puntaje[secuencia_puntaje.size()-1].array_especiales = revisar_stamps(d)
	...
	#print(d.nombre, "   ", d.get_parent().mazo_original[d.nombre].usado)
...
func _process(delta):
	if get_tree().root.has_node("Game"):
		barras = get_tree().root.get_node("Game/Barras/barras")
		puntos_shader_max = barras.get_parent().get_parent().puntos_por_nivel(barras.get_parent().get_parent().nivel_actual)*2
...
func _call_next_step(i):
	...
	if i >= secuencia_puntaje.size():
		...
		print(get_tree().root.get_node("Game").EFECTOS_TEMPORALES)
	...
	print("Shader =", valor_actual, " | Domino:", domino.name, " | Puntos sumados:", puntos_propios, " | Total mostrado:", valor_animado)
	...
	for i in paso["array_especiales"]:
		if i[1] == "especial_suma":
			i[1] = "especial"
			nodo_anim.get_parent().sumar_puntos(i[0])
		
		nodo_anim.mostrar_tipo(i[1], i[0], 2.0, [20, 50, 40], 0.4)
...
func revisar_stamps(d):
	...
	for i in d.yo.stamps:
		if i.has("Ascendant Stamp"):
			...
			array.append([2, "especial"])
			
			baraja.mazo_original[d.nombre].puntaje = puntaje+2
			d.yo.puntaje = puntaje+2
		
		
		if i.has("Golden Stamp"):
			array.append([5*Global.stats["plata_mult"]*Global.stats["plata_"+d.yo.color.to_lower()+"_mult"], "plata"])
		
		
		if i.has("Silver Stamp"):
			array.append([3*Global.stats["plata_mult"]*Global.stats["plata_"+d.yo.color.to_lower()+"_mult"], "plata"])
		
		
		if i.has("Bronze Stamp"):
			array.append([1*Global.stats["plata_mult"]*Global.stats["plata_"+d.yo.color.to_lower()+"_mult"], "plata"])
		...
		if i.has("Emerald Stamp"):
			...
			for k in get_tree().root.get_node("Game/Viewport/Baraja").get_children():
				if !k.get_node("Sprite").visible: plata_acumulada += 1
			
			array.append([plata_acumulada*Global.stats["plata_mult"]*Global.stats["plata_"+d.yo.color.to_lower()+"_mult"], "plata"])
		...
		if i.has("God Stamp"):
			...
			print("PUNTOS A DAR:     ", puntaje_a_dar)
			
			array.append([puntaje_a_dar, "especial_suma"])
