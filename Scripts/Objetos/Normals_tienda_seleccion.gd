...
var _ultimo_z_index = null


func _ready():
	_ultimo_z_index = z_index
...
func poner_stamps():
	...
	if yo.stamps != []:
		for i in yo.stamps:
			...
			instancia.position = Vector2(-250, (count*160)-120)
...
func _physics_process(delta):
	...
	var titulo_label = get_node_or_null("Descripcion/MarginContainer/Titulo/Label")
	var desc_label   = get_node_or_null("Descripcion/MarginContainer/Descripcion/Label")
	
	if titulo_label: titulo_label.bbcode_enabled = true
	
	if desc_label: desc_label.bbcode_enabled   = true
	
	if titulo_label: titulo_label.bbcode_text = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(domino_data["titulo"]))
	
	if desc_label: desc_label.bbcode_text   = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(domino_data["descripcion"]+str(domino_data["puntaje"])))
	
	
	if arrastrable == false and mouse_over:
		Global.bandera_mouse = true
		
		if Input.is_action_just_pressed("click"):
			get_tree().root.get_node("Game").insertar_stampa(self)
	
	if z_index != _ultimo_z_index:
		print("z_index cambio en", name, "de", _ultimo_z_index, "a", z_index)
		print_stack()
		_ultimo_z_index = z_index
	
	if get_parent().get_parent().get_parent().estacion_actual == "tienda" and get_parent().get_parent().get_parent().baraja_activa == "usando_amuleto":
...
func crear_puntitos():
	var mazo_a_usar = Global.dominos.duplicate()
	mazo_a_usar.merge(Global.dominos_especiales, true)
	
	$Sprite.texture = preload("res://assets/tex/dominos.png")
	$Sprite.region_enabled = true
	$Sprite.region_rect = Rect2(mazo_a_usar[nombre]["region_rect_cords"], Vector2(32, 64))
	
	var domino_data = mazo_a_usar.get(nombre, null)
	if domino_data == null:
		return
	...
	#var costo_label  = $Descripcion/MarginContainer/Costo/Label
	...
	if descripcion == "":
		titulo_label.bbcode_text = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(domino_data["titulo"]))
		desc_label.bbcode_text   = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(domino_data["descripcion"]))
		#costo_label.bbcode_text  = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(Global.stats["cost_normal_domino"]) +"[color=#b1911a]"+ Global.prefix_plata)
	else:
		var colores = {
		...
		if descripcion == "<#e0483e>Delete This Domino":
			titulo_label.bbcode_text = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode("%s <%s>%s" % [nombre.split("_")[1], colores[nombre.split("_")[0]].hex, colores[nombre.split("_")[0]].en]))
		else:
			titulo_label.bbcode_text = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode("<%s>%s" % [colores[nombre.split("_")[0]].hex, colores[nombre.split("_")[0]].en]))
		
		desc_label.bbcode_text   = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(descripcion))
		#costo_label.bbcode_text  = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(Global.stats["cost_normal_domino"]) +"[color=#b1911a]"+ Global.prefix_plata)
...
func update_focus():
	if get_parent().get_parent().get_parent().estacion_actual == "tienda" and get_parent().get_parent().get_parent().baraja_activa == "usando_amuleto":
		...
		for d in get_tree().get_nodes_in_group("dominos"):
			if d.get_parent().name == self.get_parent().name:
