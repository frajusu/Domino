	...
	if true:
		...
		for child in get_node("Sprite-top").get_children():
			if child.name in yo.flechitas:
				...
				aniadido.position = child.position
...
func _physics_process(delta):
	...
	desc_label.bbcode_text   = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(domino_data["descripcion"]+str(domino_data["puntaje"])))
...
func crear_puntitos():
	...
	if yo.stamps != []:
		for i in yo.stamps:
			...
			#Global.usar_stamp_en_domino(i[0], self, "normal")
			
			count += 1
	
	crear_lineas(numero, base_color)
...
func update_focus():
	if get_parent().get_parent().get_parent().baraja_activa == "baraja": # and !Ejecutador.ejecutando:
		var hovered = []
