...
func _ready():
	match nombre:
		"Rose Bloom":
			get_tree().root.get_node("Game").EFECTOS_TEMPORALES_AMULETOS.append(["+"+str(valores_diccionario["Rose Bloom"][1]), "rose_mult", "Rose Bloom", true])
			get_tree().root.get_node("Game").aplicar_efecto("Rose Bloom")
		
		"Domino Surge":
			for color in valores_diccionario["Domino Surge"].keys():
				get_tree().root.get_node("Game").EFECTOS_TEMPORALES_AMULETOS.append(["+"+str(valores_diccionario["Domino Surge"][color]), color+"_mult", "Domino Surge", true])
...
func _physics_process(delta):
		...
		var precio_str = get_parent().mazo_original[nombre]["plata"]
		...
		if costo_label: costo_label.bbcode_text  = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(precio_bbcode)) + "[color=#b1911a]" + Global.prefix_plata
...
func update_focus():
	if get_parent().get_parent().get_parent().get_parent().estacion_actual == "tienda" and get_parent().get_parent().get_parent().get_parent().baraja_activa != "usando_amuleto":
		var hovered = []
		...
		for d in hovered:
			if get_pos(d).x < get_pos(leftmost).x:
				leftmost = d
		
		for d in get_tree().get_nodes_in_group("dominos"):
			if !get_parent().dragging and !get_parent().get_parent().get_node("Baraja_normales").dragging and !get_parent().get_parent().get_node("Baraja_specials").dragging:
				if d == leftmost:
