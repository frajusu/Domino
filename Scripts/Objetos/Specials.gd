...
func _ready():
	visible = false
	Global.reproducir_anim_con_delay(self, "aparecer", 0.2)
	...
	$Sprite.move_child(puntitos_node, $Sprite.get_child_count())
	puntitos_node.z_index = 1
	
	for child in puntitos_node.get_children():
		child.queue_free()
	var stamps_tex = preload("res://assets/tex/stamps.png")
	...
	if yo.stamps != []:
		for i in yo.stamps:
			...
			punto.texture = stamps_tex
			punto.region_enabled = true
			punto.region_rect = Rect2(Diccionarios.stamps[i[0]]["position"], Vector2(16, 16))
			punto.position = i[1]+Vector2(6, 22.5)
			punto.material = Global.MAT_STAMP
			puntitos_node.add_child(punto)
			
			var instancia = Global.desripcion_para_stamps.instance()
			$Descripcion.add_child(instancia)
			instancia.position = Vector2(-250, (count*160)-200)
			instancia.get_node("Descripcion/Label").bbcode_text = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(Diccionarios.stamps[i[0]]["descripcion_corta"]))
			instancia.get_node("Titulo/Label").bbcode_text      = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(Diccionarios.stamps[i[0]]["titulo"]))
			
			if i[0] == "Ascendant Stamp":
				instancia.get_node("Descripcion/Label").bbcode_text = "[center]%s[/center]" % Text.parsear_colores_bbcode1(Text.parsear_colores_bbcode(Diccionarios.stamps[i[0]]["descripcion_corta"]+str(yo.puntaje)+")"))
			
			#Global.usar_stamp_en_domino(i[0], self, "especial")
	...
	if yo.titulo == "<#76357a>Mirror":
		if true:
			...
			var duplicado = duplicar_sin_slots(get_node("Sprite-top"))
			
			duplicado.material = get_node("Sprite-top").material
			duplicado.get_node("Circulito").material = get_node("Sprite-top/Circulito").material
			duplicado.get_node("Sprite-top2").material = get_node("Sprite-top/Sprite-top2").material
			
			viewport.add_child(duplicado)
			
			duplicado.position = Vector2(256, 256)/2
			...
			sprite.position = Vector2(-8, 50)
			sprite.scale = Vector2(0.5, 0.5)
			sprite.name = "fantasma"
			sprite.visible = false
			get_node("Sprite-top").add_child(sprite)
			
			for child in get_node("Sprite-top").get_children():
				if child.name in yo.flechitas:
					var aniadido = child.duplicate()
					aniadido.remove_from_group("slots")
					aniadido.scale = Vector2(2, 2)
					aniadido.self_modulate.a = 260
					aniadido.set_script(null) # 🔹 Quita el script si lo tiene
					sprite.add_child(aniadido)
					aniadido.position = Vector2(100, 100)
					aniadido.offset = child.offset
			
			sprite.material = get_tree().root.get_node("Game/Shaders/testeo/fantasma").material.duplicate()


func duplicar_sin_slots(nodo: Node) -> Node:
	var copia = nodo.duplicate() # crea una copia del nodo actual
	
	# elimina cualquier hijo duplicado automáticamente
	for hijo in copia.get_children():
		hijo.free()
	
	# agrega manualmente solo los hijos válidos
	for child in nodo.get_children():
		if not child.is_in_group("slots") or !Global.simbolos.has(child.name):
			copia.add_child(child.duplicate())
...
func desaparecer():
	...
	if actual.padre != null and (typeof(actual.padre) != TYPE_INT):
...
func setear_posision_correcta():
	if yo.titulo == "<#76357a>Mirror":
		#print(get_node("Sprite-top").get_children())
		for i in get_parent().get_children()+get_parent().get_parent().get_parent().get_node("Baraja").get_children():
			if i.get_node("Sprite-top").has_node("fantasma"):
				i.get_node("Sprite-top/fantasma").visible = false
				
				for k in i.get_node("Sprite-top/fantasma").get_children():
					k.visible = false
	
	if $Sprite.visible == false:
		if yo.titulo == "<#76357a>Mirror":
			for i in get_parent().get_all_descendants(self):
				if !(i.get_parent().name != "Baraja_S" or i.yo.titulo == "<#76357a>Mirror"):
					break
				
				if i.get_node("Sprite-top").has_node("fantasma"):
					i.get_node("Sprite-top/fantasma").visible = true
					i.get_node("Sprite-top/fantasma").position.y = 10
					i.get_node("Sprite-top/fantasma").show_behind_parent = true
					i.get_node("Sprite-top").move_child(i.get_node("Sprite-top/fantasma"), 0)
				
				
				for k in i.get_slots_in_self():
					if k.hijo != null:
						i.get_node("Sprite-top/fantasma").get_node(k.name).position = Vector2(k.position.x, k.position.y)*2
						i.get_node("Sprite-top/fantasma").get_node(k.name).use_parent_material = true
						i.get_node("Sprite-top/fantasma").get_node(k.name).visible = false
	
	
	if posision_fija != Vector2.ZERO and get_parent().dragging != self:
		global_position = posision_fija
	
	if padre != null and (typeof(padre) != TYPE_INT):
		for i in padre.get_slots_in_self():
			if i.hijo == self:
				global_position = i.global_position + Vector2(0, -0.8)
				for k in get_slots_in_self():
					k.scale.x = i.scale.x
					k.position.x = k.posision_original*i.scale.x
		...
		if padre == null and get_parent().dragging != self:
			...
			for k in get_slots_in_self():
				k.scale.x = k.scale_original
	
	if get_node("Sprite").visible:
		posision_fija = Vector2.ZERO


func _physics_process(delta):
	setear_posision_correcta()
...
func actualizar_sombra():
	...
	if sprite.visible:
		$Descripcion.position.y = -73
	else:
		if $top/VisibilityNotifier2D.is_on_screen() == true:
			$Descripcion.position.y = -33
		else:
			$Descripcion.position.y = 200
			pass
...
func update_focus():
	if true or !Ejecutador.ejecutando:
