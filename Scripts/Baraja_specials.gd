				...
				if event.pressed:
					...
					if dragging:
						if dragging.get_node("Sprite").visible:
							...
							if dragging.padre != null and (typeof(dragging.padre) != TYPE_INT or dragging.padre != 1):
								dragging.padre.hijo = null
				...
				if event.pressed:
					for s in get_children():
						if s != arrastrado:
							# Verificamos si el mouse está dentro del "círculo" del sprite
							if s.mouse_dentro_area():
								if s.padre != null and (typeof(s.padre) != TYPE_INT or s.padre != 1):
									s.padre.get_node("Sprite-top/Flechita").visible = false
...
#				dragging.get_node("Sprite-top").rotation_degrees += 5
			...
			var _hijo_nombre = "null"
			if dragging.hijo != null:
				_hijo_nombre = dragging.hijo.nombre
			...
			for s in slots:
				...
				if domino_slot.hijo == null and domino_slot.get_node("Sprite").visible == false:
					s.visible = false
			...
			for a in dominos:
				for b in dominos:
					...
					if dist < distancia_de_repelerse and dist > 0.001:
						...
						if a.get_parent().name == "Baraja_S":
							a.global_position += dir * corr
						else:
							a.global_position += dir * corr
						
						if b.get_parent().name == "Baraja_S":
				...
				if dragging.get_node("Sprite").visible == false:
					...
					for s in slots:
						...
						if !domino_slot.esta_en_cadena(dragging):
							if domino_slot.hijo == null and domino_slot.get_node("Sprite").visible == false and domino_slot != dragging:
								s.visible = true
					
					if dragging.hijo == null:
						dragging.get_node("Sprite-top/Flechita").visible = false
					
					if dragging.hijo == null or true:
						for s in slots:
							if s != dragging.get_node("Sprite-top/Flechita"):
								var domino_slot = s.get_parent().get_parent()
								
								var dist = dragging.global_position.distance_to(s.global_position)
								
								if dist < lock_distance:
									if domino_slot.hijo == null or domino_slot.hijo == dragging:
										dragging.global_position = s.global_position
									
									if domino_slot.hijo == null:
										var raiz = dragging.obtener_raiz()
										if !domino_slot.esta_en_cadena(raiz):
											domino_slot.hijo = dragging
											dragging.padre = domino_slot
								else:
									if domino_slot.hijo == dragging:
										domino_slot.hijo = null
										dragging.padre = null
