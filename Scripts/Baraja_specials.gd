...
var domino = preload("res://Scenas/Specials.tscn")
...
func _ready():
	...
	var _a = get_tree().root.get_node("Game/Viewport/GameArea/Area2D").connect("mouse_entered", self, "_on_mouse_entered")
	_a = get_tree().root.get_node("Game/Viewport/GameArea/Area2D").connect("mouse_exited", self, "_on_mouse_exited")
...
func setear_posision_correcta():
	if dragging != null:
		for i in get_all_descendants(dragging):
			if "posision_fija" in i:
				i.posision_fija = Vector2.ZERO

#		if i.posision_fija != Vector2.ZERO and get_parent().dragging != self:
...
#
#						posision_fija = i.global_position + Vector2(0, -0.5)
#						global_position = i.global_position + Vector2(0, -0.5)
#
...
func draw_cards(n):
	...
	for _i in range(n):
		...
		#var lineas = strip_bbcode(desc_label.text).length()/15
		#print(lineas)
		#s.get_node("Descripcion/MarginContainer/Descripcion/real").rect_size.y = lineas*4
		#s.get_node("Descripcion/MarginContainer/Descripcion/real/sombra").rect_size.y = lineas*4
		#desc_label.rect_position.y = s.get_node("Descripcion/MarginContainer/Descripcion/real").rect_position.y-s.get_node("Descripcion/MarginContainer/Descripcion/real/sombra").rect_size.y*6
...
func strip_bbcode(bbcode: String) -> String:
	var resultado := ""
	var dentro := false
	for c in bbcode:
		if c == "[":
...
#				if dragging != s:
#					if s.bandera:
#						for i in get_all_descendants(s):
#							if i.posision_fija != Vector2.ZERO:
#								i.posision_fija = i.posision_fija-Vector2(1, 0)
#
#						s.bandera = false
#
#					s.scale = Vector2(1,1)
...
func _input(event):
	#if get_parent().get_parent().get_parent().baraja_activa == "baraja_especial":
				...
				if event.pressed:
					for s in get_children():
						if s != arrastrado:
							...
							if s.mouse_dentro_area() and !get_parent().get_parent().get_node("Baraja").dragging:
								dragging = s
...
#				dragging.get_node("Sprite-top").rotation_degrees += 5
			...
			if dragging.padre != null:
				if (typeof(dragging.padre) != TYPE_INT or dragging.padre != 1):
			...
			if Global.usar_offset:
				if dragging.get_node("Sprite").visible:
					...
					dragging.global_position = get_global_mouse_position() + Vector2(0.5, 0)
			else:
				if dragging.get_node("Sprite").visible:
		...
		elif event is InputEventMouseMotion and !get_parent().get_parent().get_node("Baraja").dragging:
				...
				for s in slots:
					...
					if dragging.yo.padre != 2:
						if !domino_slot.esta_en_cadena(dragging):
							if s.hijo == null and domino_slot.get_node("Sprite").visible == false and domino_slot != dragging:
								s.visible = true
