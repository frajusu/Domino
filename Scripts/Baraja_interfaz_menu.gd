...
func draw_cards(color: String) -> void:
		...
		# Filtramos solo las cartas que sean del color pedido
		var cartas_color := []
		for nombre in keys:
			if nombre.begins_with(color + "_"):
		...
		# Reducir draw
		if keys.size() != Global.dominos.keys().size():
			get_parent().get_parent().draws_actuales -= 1
		
		# Limitar para no pasarse de la mano máxima
		var espacio_disponible = Global.stats.max_cards_in_hand - current_cards
		if cartas_color.size() > espacio_disponible:
			print("Solo se pueden agregar %d cartas para no pasar de %d" % [espacio_disponible, Global.stats.max_cards_in_hand])
			cartas_color = cartas_color.slice(0, espacio_disponible)
		
		# Crear todas las cartas del color elegido
		...
		print("Cartas restantes en el mazo: ", mazo_actual.keys().size())
