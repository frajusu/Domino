...
var niveles = {
	"default" : {
		...
		"descripcion" : "A normal, lame level with Normal Difficulty",
		"size" : Vector2(32, 32),
	...
	"boss_ejemplo" : {
		...
		"descripcion" : "It looks like a special level, even though it’s as lame as a normal one.",
		"size" : Vector2(32, 32),
...
func reproducir_sonido(tipo_sonido: String, _posision_del_sonido, mult = 1.0):
	var sonidos = {
		...
		"mult_retro_magic"  : ["res://assets/SFX/mult_retro_magic.wav",  -10,  0, 1, "eff"],
		"mult_chips_gather" : ["res://assets/SFX/mult_chips_gather.wav", -10, 10, 1, "eff"],
		
		"especial_retro_pick" : ["res://assets/SFX/especial_retro_pick.wav", -10, 10, 1, "eff"],
...
func _physics_process(_delta):
	if frame_print < 30:
		...
		frame_print = 0
		#print("contadores_amuletos   ", Global.contadores_amuletos, "     contadores_ejecuciones_amuletos    ", contadores_ejecuciones_amuletos)
	
	musica(false)
	
	if get_tree().current_scene:
		if get_tree().current_scene.name == "generador_imagen":
			if Input.is_action_just_pressed("click"):
				var img = get_viewport().get_texture().get_data()
				img.flip_y() # MUY importante
				
				var ruta = "user://captura.png"
				var err = img.save_png(ruta)
				
				if err == OK:
					print("Guardado en:", ProjectSettings.globalize_path(ruta))
				else:
					print("Error al guardar PNG")
	...
	if get_tree().root.has_node("Game") or get_tree().root.has_node("Menu_Principal"):
		if !sonido.eff:
			get_tree().current_scene.get_node("wind").volume_db = -80
...
func _ready():
	...
	var ruta_base = get_environment() + "/Falaro"
	...
	if error != OK:
		...
		save.set_value("Partida", "Mazo_valores_amuletos",[])
		...
		save.set_value("Misc",     "endless",      Global.endless)
		save.save(ruta)
		...
		Global.endless      = Global.leer_save("Misc",     "endless")
		print("Archivo ya existe")
	
	
	if get_tree().current_scene.name == "Game" or get_tree().current_scene.name == "Menu_Principal" or get_tree().current_scene.name == "intro":
		musica(true)
...
func leer_save(seccion: String, nombre: String, defecto := null):
	var ruta = get_environment() + "/Falaro/save.cfg"
	var save = ConfigFile.new()
...
func guardar_save(seccion: String, nombre: String, valor):
	...
	if error != OK:
		var dir = Directory.new()
		var carpeta = get_environment() + "/Falaro"
	...
	for color in colores.keys():
		...
		for numero in range(1, 9):  # del 1 al 8
			...
			dominos[nombre] = {
				...
				"descripcion": "This domino adds <%s>+" % [azul] # ejemplo: "This domino adds <#f3983a>+1"
			}
...
var cola_amuletos = []
var animacion_en_proceso = false


var setear_fifth = true


func usar_amuleto_animacion(_amuleto):
	cola_amuletos.append(_amuleto)
	_intentar_reproducir_siguiente()


var repetido = false


func _intentar_reproducir_siguiente():
	var poder_sumar_al_contador = true
	
	if animacion_en_proceso:
		return
	if cola_amuletos.empty():
		return
	
	animacion_en_proceso = true
	var amuleto = cola_amuletos.pop_front()
	
	
	if repeticiones_fifth_pendientes > 0:
		repeticiones_fifth_pendientes -= 1
		poder_sumar_al_contador = false
		cola_amuletos.push_front(amuleto)
	
	if repetido: poder_sumar_al_contador = false
	
	#print("poder_sumar_al_contador      ", poder_sumar_al_contador)
	
	var amuleto_a_usar = amuleto.replace("1", "")
	
	if amuleto.ends_with("1"):
		amuleto = "Crystal Dice"
	
	
	match amuleto_a_usar:
		
		#ACA AUMENTAMOS LAS STATS COMO GLOBAL
		
		"Giant's Tear Drop":
			Ejecutador.convertir_a_plata = true
			if poder_sumar_al_contador: contadores_ejecuciones_amuletos += 1
		
		
		"Gold Seed":
			var mult = int((get_tree().root.get_node("Game").plata)/50)
			
			if mult >= 1:
				var multiplicador = pow(2, mult)
				Global.stats.global_mult *= int(multiplicador)
				get_tree().root.get_node("Game").actualizar_global_mult()
				if poder_sumar_al_contador: contadores_ejecuciones_amuletos += 1
			else:
				animacion_en_proceso = false
				_intentar_reproducir_siguiente()
				if poder_sumar_al_contador: contadores_amuletos += 1
				return
		
		"Gold Tree":
			var mult = int((get_tree().root.get_node("Game").plata)/100)
			
			if mult >= 1:
				var multiplicador = pow(5, mult)
				Global.stats.global_mult *= int(multiplicador)
				get_tree().root.get_node("Game").actualizar_global_mult()
				if poder_sumar_al_contador: contadores_ejecuciones_amuletos += 1
			else:
		...
		"Fading Luck":
			...
			var amuleto_nodo  = get_tree().root.get_node("Game/Viewport/Zona_de_interfaz/xd/Zona_de_amuletos/Baraja_S/ScrollContainer/MarginContainer/GridContainer").get_child(contadores_amuletos)
			var amuleto1_nodo = get_tree().root.get_node("Game/Viewport/Zona_de_interfaz/Tienda/Zona_de_amuletos/Baraja_S/ScrollContainer/MarginContainer/GridContainer").get_child(contadores_amuletos)
			
			if int(amuleto_nodo.valores_diccionario["Fading Luck"][1]) != 0:
				var paso = Ejecutador.secuencia_puntaje[Ejecutador.secuencia_puntaje.size()-1]
				nodo_anim.global_position = paso["posicion"] + Vector2(0, -30)
				
				nodo_anim.scale = Vector2(0.5, 0.5)
				nodo_anim.mostrar_tipo("suma_mute", amuleto_nodo.valores_diccionario["Fading Luck"][1], 2.0, [20, 50, 40], 0.4)
				
				#yield(get_tree().create_timer(0.1), "timeout") 
				
				Ejecutador.valor_animado += amuleto_nodo.valores_diccionario["Fading Luck"][1]
				
				var actual = amuleto_nodo.valores_diccionario["Fading Luck"][1]
				
				amuleto_nodo.valores_diccionario["Fading Luck"][1]  = actual-10.0
				amuleto1_nodo.valores_diccionario["Fading Luck"][1] = actual-10.0
				
				print(amuleto_nodo.valores_diccionario["Fading Luck"][1])
		...
		"Puzzle Piece":
			...
			for k in Ejecutador.secuencia_puntaje:
				contadores[k["domino"].nombre] = 0
			
			for k in Ejecutador.secuencia_puntaje:
				contadores[k["domino"].nombre] += 1
		...
		"Chain Lock":
			var color_anterior = null
			var hay_seguidos = false
			
			for k in Ejecutador.secuencia_puntaje:
				var color_actual = k["domino"].yo.color
				
				if color_anterior != null and color_actual == color_anterior:
					hay_seguidos = true
					break
			...
			if hay_seguidos:
				...
				if poder_sumar_al_contador: contadores_ejecuciones_amuletos += 1
				get_tree().root.get_node("Game").recibir_plata("Domino: ", int(10))
		
		
		"Chain Dealer":
			var excedente = Ejecutador.secuencia_puntaje.size() - 6
			
			if excedente > 0:
				var multiplicador = pow(2, excedente)
				Global.stats.global_mult *= int(multiplicador)
				get_tree().root.get_node("Game").actualizar_global_mult()
				if poder_sumar_al_contador:
					contadores_ejecuciones_amuletos += 1
			else:
				animacion_en_proceso = false
				_intentar_reproducir_siguiente()
				if poder_sumar_al_contador:
		...
		"Sun Pendant":
			var cont = float(get_tree().root.get_node("Game").amuletos_tenidos.size())
			
			if cont > 0.0:
				Global.stats.global_mult += cont
				get_tree().root.get_node("Game").actualizar_global_mult()
				if poder_sumar_al_contador: contadores_ejecuciones_amuletos += 1
			else:
				animacion_en_proceso = false
		...
		"Amber Core":
			var hacer = Ejecutador.secuencia_puntaje[0]["domino"].yo.color.to_lower() == "orange" or Ejecutador.secuencia_puntaje[0]["domino"].yo.color.to_lower() == "all"
			
			if hacer:
				if poder_sumar_al_contador: contadores_ejecuciones_amuletos += 1
				
				for i in get_tree().root.get_node("Game/Viewport/Baraja").mazo_original.keys():
					var aumentar = (get_tree().root.get_node("Game/Viewport/Baraja").mazo_original[i].color.to_lower()) == "orange" or (get_tree().root.get_node("Game/Viewport/Baraja").mazo_original[i].color.to_lower()) == "all"
					
					if aumentar:
						get_tree().root.get_node("Game/Viewport/Baraja").mazo_original[i].puntaje += 2
			else:
				animacion_en_proceso = false
				_intentar_reproducir_siguiente()
				if poder_sumar_al_contador: contadores_amuletos += 1
		...
		"Stack Overflow":
			...
			for i in Ejecutador.secuencia_puntaje:
				i = i["domino"].nombre
				
				var i_nuevo = i+"t"
				
				get_tree().root.get_node("Game/Viewport/Baraja").mazo_original[i_nuevo] = get_tree().root.get_node("Game/Viewport/Baraja").mazo_original[i].duplicate()
			
			get_tree().root.get_node("Game/Viewport/Menu_info/Cartas_normales/Rojo").mazo_original     = get_tree().root.get_node("Game/Viewport/Baraja").mazo_original.duplicate()
			get_tree().root.get_node("Game/Viewport/Menu_info/Cartas_normales/Azul").mazo_original     = get_tree().root.get_node("Game/Viewport/Baraja").mazo_original.duplicate()
			get_tree().root.get_node("Game/Viewport/Menu_info/Cartas_normales/Verde").mazo_original    = get_tree().root.get_node("Game/Viewport/Baraja").mazo_original.duplicate()
			get_tree().root.get_node("Game/Viewport/Menu_info/Cartas_normales/Amarillo").mazo_original = get_tree().root.get_node("Game/Viewport/Baraja").mazo_original.duplicate()
		
		
		#(d.yo.color.to_lower() == "blue" or d.yo.color.to_lower() == "all"):
		...
		"Overgrowth":
			...
			for i in Ejecutador.secuencia_puntaje:
				var color = (i["domino"].yo.color.to_lower())
			...
			for i in colores.keys():
				if colores[i] >= 5:
					...
					get_tree().root.get_node("Game").EFECTOS_TEMPORALES_AMULETOS.append(["+10", i+"_%", "Overgrowth", true])
					
					var actual = amuleto_nodo.valores_diccionario["Overgrowth"][i]
					
					amuleto_nodo.valores_diccionario["Overgrowth"][i]  = actual+10
					amuleto1_nodo.valores_diccionario["Overgrowth"][i] = actual+10
		...
		"Black Ink":
			...
			for i in get_tree().root.get_node("Game/Viewport/Baraja").mazo_original.keys():
				var color = (get_tree().root.get_node("Game/Viewport/Baraja").mazo_original[i].color.to_lower())
			...
			var a = Global.stats.global_mult
			
			for i in colores.keys():
				if colores[i] == 0:
					Global.stats.global_mult += 5
					get_tree().root.get_node("Game").actualizar_global_mult()
			
			if Global.stats.global_mult == a:
				animacion_en_proceso = false
				_intentar_reproducir_siguiente()
				if poder_sumar_al_contador: contadores_amuletos += 1
				return
		...
		"Combo Trinity":
			...
			for i in Ejecutador.secuencia_puntaje:
				var numero = (i["domino"].yo.titulo.split(" ")[0])
			...
			for k in numeros:
				if k != "7" and k != "8" and numeros[k] > 0:
					encontro = true
					break
			
			if !encontro:
				Global.stats.global_mult += 3
				get_tree().root.get_node("Game").actualizar_global_mult()
				if poder_sumar_al_contador: contadores_ejecuciones_amuletos += 1
			else:
				animacion_en_proceso = false
		...
		"Domino Surge":
			...
			for i in colores.keys():
				if colores[i] >= 5:
					...
					get_tree().root.get_node("Game").EFECTOS_TEMPORALES_AMULETOS.append(["+0.5", i+"_mult", "Domino Surge", true])
					
					var actual = amuleto_nodo.valores_diccionario["Domino Surge"][i]
					
					amuleto_nodo.valores_diccionario["Domino Surge"][i]  = actual+0.5
					amuleto1_nodo.valores_diccionario["Domino Surge"][i] = actual+0.5
	...
	nodo.get_node("Sprite").region_rect.position = Diccionarios.amuletos[amuleto].position
	
	var anim = nodo.get_node("AnimationPlayer")
	
	if not anim.is_connected("animation_finished", self, "_on_animacion_terminada"):
		anim.connect(
			"animation_finished",
			self,
			"_on_animacion_terminada",
			[amuleto]   # <<< se bindea aca
		)
...
var contadores_ejecuciones_amuletos = 0

var repeticiones_fifth_pendientes = 0


func cantidad_fifth():
	var c = 0
	var contador_general = 0
	for a in get_tree().root.get_node("Game").amuletos_tenidos:
		if a == "Fifth is the charm":
			c += 1
		
		elif a == "Crystal Dice":
			var amuleto_nodo  = get_tree().root.get_node("Game/Viewport/Zona_de_interfaz/xd/Zona_de_amuletos/Baraja_S/ScrollContainer/MarginContainer/GridContainer").get_child(contador_general)
			
			if amuleto_nodo.valores_diccionario["Crystal Dice"][1] == "Fifth is the charm":
				c += 1
...
func cantidad_Augmented_plus_1():
	var c = 1
	var contador_general = 0
	for a in get_tree().root.get_node("Game").amuletos_tenidos:
		if a == "Augmented Chance":
			c += 1
		
		elif a == "Crystal Dice":
			var amuleto_nodo  = get_tree().root.get_node("Game/Viewport/Zona_de_interfaz/xd/Zona_de_amuletos/Baraja_S/ScrollContainer/MarginContainer/GridContainer").get_child(contador_general)
			
			if amuleto_nodo.valores_diccionario["Crystal Dice"][1] == "Augmented Chance":
				c += 1
...
var se_ejecuto_coso_ultimo = false


func _on_animacion_terminada(_nombre, amuleto):
	animacion_en_proceso = false
	
	if contadores_ejecuciones_amuletos > 0 \
	and (contadores_ejecuciones_amuletos) % 5 == 0 \
	and get_tree().root.get_node("Game").amuletos_tenidos.has("Fifth is the charm") \
	and !se_ejecuto_coso_ultimo and !repetido:
		
		if setear_fifth:
			repeticiones_fifth_pendientes = cantidad_fifth()
		...
		# animacion Fifth
		var nodo = get_tree().get_nodes_in_group("camera")[0].get_node("uso_amuleto_anim")
		nodo.get_node("Sprite").region_rect.position = Diccionarios.amuletos["Fifth is the charm"].position
		...
		var anim = nodo.get_node("AnimationPlayer")
		if not anim.is_connected("animation_finished", self, "_on_animacion_terminada"):
			anim.connect("animation_finished", self, "_on_animacion_terminada")
		
		anim.play("anim")
		
		animacion_en_proceso = false
		
		se_ejecuto_coso_ultimo = true
		
		return
	
	
	var rng_trick = RandomNumberGenerator.new()
	rng_trick.randomize()
	
	
	if get_tree().root.get_node("Game").amuletos_tenidos.has("Trickster's Bloom") and rng_trick.randi_range(1, int(10/Global.cantidad_Augmented_plus_1())) == 1 and !se_ejecuto_coso_ultimo and !repetido:
		if setear_trick:
			cola_amuletos.push_front(amuleto)
		...
		# animacion Fifth
		var nodo = get_tree().get_nodes_in_group("camera")[0].get_node("uso_amuleto_anim")
		nodo.get_node("Sprite").region_rect.position = Diccionarios.amuletos["Trickster's Bloom"].position
	...
	if get_tree().root.get_node("Game").amuletos_tenidos.has("Crystal Shard") and !se_ejecuto_coso_ultimo and !repetido:
		if setear_crystal:
			cola_amuletos.push_front(amuleto)
			
			setear_crystal = false
		
		# animacion Fifth
		var nodo = get_tree().get_nodes_in_group("camera")[0].get_node("uso_amuleto_anim")
		nodo.get_node("Sprite").region_rect.position = Diccionarios.amuletos["Crystal Shard"].position
	...
	if se_ejecuto_coso_ultimo:
		se_ejecuto_coso_ultimo = false
		repetido = true
	else:
		setear_crystal = true
		setear_trick = true
...
var setear_crystal = true

var setear_trick = true
