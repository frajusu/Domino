...
func _physics_process(_delta):
	if get_tree().root.has_node("Game"):
		get_tree().get_nodes_in_group("camera")[0].get_parent().get_node("camara").visible = Global.overlay
		if !Global.overlay_default:
			get_tree().get_nodes_in_group("camera")[0].get_parent().get_node("camara").material.set_shader_param("vignette_opacity", Global.vignete_opacity)
			get_tree().get_nodes_in_group("camera")[0].get_parent().get_node("camara").material.set_shader_param("warp_amount", Global.wrap)
			get_tree().get_nodes_in_group("camera")[0].get_parent().get_node("camara").material.set_shader_param("static_noise_intensity", Global.noise)
			get_tree().get_nodes_in_group("camera")[0].get_parent().get_node("camara").material.set_shader_param("posterize_colors", Global.reducir_colores)
		else:
