...
func _intentar_reproducir_siguiente():
	...
	match amuleto:
		...
		"Amber Core":
			var hacer = Ejecutador.secuencia_puntaje[0]["domino"].yo.color.to_lower() == "orange"
			
			if hacer:
				for i in get_tree().root.get_node("Game/Viewport/Baraja").mazo_original.keys():
					var aumentar = (get_tree().root.get_node("Game/Viewport/Baraja").mazo_original[i].color.to_lower()) == "orange"
...
func _on_animacion_terminada(_nombre):
	animacion_en_proceso = false
	_intentar_reproducir_siguiente()
