...
func _physics_process(_delta):
	...
	if viewport_zize.x == 0 and viewport_zize.y != 0:
		$Tex_mira.scale = (Vector2(0 , 576/viewport_zize.y))/4
	else:
		if viewport_zize.y == 0 and viewport_zize.x != 0:
			$Tex_mira.scale = (Vector2(1024/viewport_zize.x , 0))/4
		else:
			if viewport_zize.y == 0 and viewport_zize.x == 0:
				$Tex_mira.scale = (Vector2(0 , 0))/4
			else:
				$Tex_mira.scale = (Vector2(1024/viewport_zize.x , 576/viewport_zize.y))/4
